...
function createFormsTabGroups(element, bpmnFactory, elementRegistry, translate) {
  ...
  if(ensureFormKeyAndDataSupported(element)){
    ...
    if(element.businessObject.formKey){
      button = "<button class=\"custom_button\" onClick=\"document.getElementById('editorCreateForm').click();\">Gerenciar formulário Externo</button>";
    }else{
      button = "<button onClick=\"document.getElementById('editorCreateForm').click();\">Criar formulário Externo</button>";
    }
    elementRegistry._eventBus.fire("sigmin.open.form",{element:element});

    formGroup.entries.push({id:"custom_form_open",html:button},);
