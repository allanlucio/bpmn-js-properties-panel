...
function createFormsTabGroups(element, bpmnFactory, elementRegistry, translate) {
  ...
  // console.log(element, bpmnFactory, elementRegistry, translate)

  if(ensureFormKeyAndDataSupported(element)){
    var button;
    if(element.businessObject.formKey){
      button = {id:"sigmin-create-button",html:"<button class=\"custom_button\" onClick=\"document.getElementById('editorCreateForm').click();\">Gerenciar formulário Externo</button>"};
    }else{
      button = {id:"sigmin-edit-button", html:"<button onClick=\"document.getElementById('editorCreateForm').click();\">Criar formulário Externo</button>"};
    }
    elementRegistry._eventBus.fire("sigmin.open.form",{element:element});

    formGroup.entries.push(button);


  }else{
    elementRegistry._eventBus.fire("sigmin.close.form",{element:element});
  }
